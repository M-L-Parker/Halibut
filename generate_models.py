...
def main():
    ...
    for density in densities:
        ...
        for element in elements:
            ...
            element_filename = 'time_dependent_ions/ion_concs_%s_%s_%s.npz' % (
                ''.join(lc_name.split('.')[:-1]), element, str(density))
        ...
        # Can't convert to array because different sizes. Slightly irritating.
        concentrations = [np.load(x)['concentrations'] for x in filenames]

        commands = []
        filenames = []
        for i, t in enumerate(times[:-1]):
            countrate = lightcurve[i]
            # I have no idea where this number came from, Ciro wrote it down once.
            pow_norm = countrate / mean_cr * 12.566
            ...
            if not os.path.exists(run_settings.spectra_dir + 'density_%s_spectrum_%s.qdp' % (str(density), str(i))) or run_settings.clobber:
                ...
                for j, element in enumerate(elements):
                    ...
                    if len(elemental_concentrations) >= 10:
                        for k, conc in enumerate(elemental_concentrations):
                            if conc > 0:
                                # print conc, column,float(conc)*float(column)
                                tstep_commands.append('par 1 1 ' + element.lower() + '0' * (2 - len(
                                    str(k + 1))) + str(k + 1) + ' v ' + str(np.log10(float(conc) * float(column))))
                    else:
                        for k, conc in enumerate(elemental_concentrations):
                            if conc > 0:
                                tstep_commands.append('par 1 1 ' + element.lower() + str(
                                    k + 1) + ' v ' + str(np.log10(float(conc) * float(column))))
                commands.append(tstep_commands)
                filenames.append(run_settings.spectra_dir +
                                 'density_%s_spectrum_%s' % (str(density), str(i)))
