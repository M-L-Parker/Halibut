...
if __name__ == '__main__':
	...
	for element in elements:
		...
		fig=pl.figure(element+' ion abundances',figsize=(6,6))
		...
		for i,density in enumerate(densities[::-1]):
			...
			print(concentrations.shape[1],'ions, plotting H-like')
			# print concentrations[0,:]
			# exit()

			ax2.plot(zero_times[:-1],concentrations[:,-2]/np.sum(concentrations[0,:]),lw=1,color=colours[i],label=density*1.e7)
		...
		# pl.show()
		ax2.set_ylabel(element+' '+roman.toRoman(concentrations.shape[1]-1)+' abundance')
		...
		pl.savefig(element+'_ion_abundances_individual.pdf',bbox_inches='tight')
